  ...
  switch (action.type) {
    case consts.SET_GAME_MODE:
      if (action.currentState === consts.GameState.NOT_STARTED){
        ...
        return  Object.assign({}, state, {currentState: action.currentState, gameId: action.gameId});
      }

    case consts.MAKE_FIRE:
      if (state.currentState === consts.GameState.STARTED){
        const enemyBoard  = state.enemyBoard.slice();
        action.fireResult.result ?  enemyBoard[action.fireResult.cellId] = 2 : enemyBoard[action.fireResult.cellId] = 1;
        return Object.assign({}, state, {enemyBoard, isServerTurn: action.fireResult.serverturn});
      }
      return state;
    ...
    case consts.MAKE_FIRE_SERVER:
      if (state.currentState === consts.GameState.STARTED){
        ...
        isHit ? myBoard[action.fireRequest.cellId] = 3 : myBoard[action.fireRequest.cellId] = 2;

        if (!isHit){
          return Object.assign({}, state, {myBoard, isServerTurn: isHit, lastMyDestroyedShip: null});
        }
        ...
        //check ship destoyed
        const lastMyDestroyedShip = newShip !== null && newShip.cells.length == newShip.hits ? newShip : null;

        return Object.assign({}, state, {myBoard, myShips, isServerTurn: isHit, lastMyDestroyedShip});
      }
      return state;
    ...
    case consts.SHIP_ROTATED:
      if (state.currentState === consts.GameState.NOT_STARTED){
        ...
        return Object.assign({}, state, {myShips: newShips, myBoard: newBoard});
      }
      return state;
