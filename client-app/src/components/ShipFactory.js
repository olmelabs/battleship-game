...
class ShipFactory extends React.Component {
  ...
  onRotateShipClick(){
    if (this.props.currentState === consts.GameState.STARTED || this.props.currentState === consts.GameState.COMPLETED ||  this.props.myShipsCurrent < 0){
      return;
    }
  ...
  render() {
    return (
      ...
      <div className="centered text-center">
        <p>Setup your fleet. Click on round buttons to setup ships. Click on cell on the board to place ship.</p>
        {this.props.myShips.map((ship, index) =>
          <ShipDefinition key={index} index={index} ship={ship} currentShip={this.props.myShipsCurrent}/>
        )}
      </div>
      <div className="container">
          <div className="centered text-center">
            <button className="round-button" title="Rotate Ship" onClick={this.onRotateShipClick}><i className="fa fa-redo fa-2x"/></button>
          </div>
      </div>
      <div className="centered text-center">
        <p>Or click the button below to quickly generate board.</p>
        <button className="control-button btn btn-primary" title="Generate"  onClick={this.onGenerateBoardClick}>Generate</button>
      </div>
      </React.Fragment>
    );
  }
